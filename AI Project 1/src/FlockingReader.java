...
public class FlockingReader extends DefaultHandler {
    ...
    /* Allows us to allocate unique identifies to new objects */
    static int nextId = 1;
    ...
    throws SAXException
    {
        ...
        } else if (ReactiveGrazer.XML_NAME.equals(name)) {
            if (inDefaults) {
                ReactiveGrazer.updateDefaults(atts, locator);
            } else {
                ReactiveGrazer g = new ReactiveGrazer(world, id, atts, locator);
                world.addAgent(g);
            }
        } else if (StateGrazer.XML_NAME.equals(name)) {
            if (inDefaults) {
                StateGrazer.updateDefaults(atts, locator);
            } else {
                StateGrazer g = new StateGrazer(world, id, atts, locator);
                world.addAgent(g);
            }
        } else {
